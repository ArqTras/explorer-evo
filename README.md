# Evolution Onion Blockchain Explorer

Currently available Evolution blockchain explorers have several limitations which are of
special importance to privacy-oriented users:
 ...
 - do not support Evolution testnet nor stagenet networks,
 - have limited JSON API.


In this example, these limitations are addressed by development of
an Evolution Onion Blockchain Explorer. The example not only shows how to use
Evolution C++ libraries, but also demonstrates how to use:
...
## Evolution Onion Blockchain Explorer features

The key features of the Evolution Onion Blockchain Explorer are:
 ...
 - showing public components of Evolution addresses,
 - decoding which outputs and mixins belong to the given Evolution address and viewkey,
 - can prove that you send Evolution to someone,
 - detailed information about ring members, such as, their age, timescale and their ring sizes,
 - showing number of amount output indices,
 - support Evolution testnet and stagnet networks,
...
Download and compile recent Evolution into your home folder:

```bash
# first install Evolution dependecines
sudo apt update
...
Once the Evolution is compiles, the explorer can be downloaded and compiled
as follows:

```bash
# go to home folder if still in ~/evolution
cd ~
...
# altearnatively can use: cmake -DEVOLUTION_DIR=/path/to/evolution_folder ..
# if evolution is not in ~/evolution
#
...
Obtaining current Evolution emission amount is not straight forward. Thus, by default it is
...
Every 10000 blocks, the thread will save current emission in a file, by default,
 in `~/.evolution/lmdb/emission_amount.txt`. For testnet or stagenet networks,
 it is `~/.evolution/testnet/lmdb/emission_amount.txt` or `~/.evolution/stagenet/lmdb/emission_amount.txt`. This file is used so that we don't
 need to rescan entire blockchain whenever the explorer is restarted. When the
 explorer restarts, the thread will first check if `~/.evolution/lmdb/emission_amount.txt`
...
```
Evolution emission (fees) is 14485540.430 (52545.373) as of 1313448 block
```

The values given, can be checked using Evolution daemon's  `print_coinbase_tx_sum` command.
...
```bash
# we use here official Evolution project's donation address as an example
...
```json
{
  "data": {
    ...
    "git_branch_name": "update_to_current_evolution",
    "last_git_commit_date": "2017-07-25",
    "last_git_commit_hash": "a549f25",
    "evolution_version_full": "0.10.3.1-ab594cfe"
  },
  "status": "success"
}
```
...
Return raw json block data, as represented in Evolution.
...
Return raw json tx data, as represented in Evolution.
